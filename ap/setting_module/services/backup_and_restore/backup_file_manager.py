...
class BackupKey(BaseModel):
    """Backup file key for managing backup data files.

    Represents a backup file with process ID and time range, providing methods
    for file operations and path management.

    Attributes:
        FIlE_EXTENSION: File extension for backup files (Parquet).
        FILE_SEP: File separator for backup files (TSV).
        FILE_FORMAT: Date format string for file naming.
        process_id: ID of the process being backed up.
        start_time: Start time of the backup period.
        end_time: End time of the backup period.

    Methods:
        from_file_name: Create BackupKey instance from file name.
        backup_folder: Get the backup folder path.
        filename: Get the full file path.
        make_backup_dir: Create backup directory if it doesn't exist.
        delete_file: Delete the backup file.
        read_file: Read backup data from file.
        write_file: Write backup data to file.
