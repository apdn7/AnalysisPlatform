...
class SSEStreamer:
    ...
    def __init__(self, blocking_message_timeout: int = 1, idle_timeout: float = 60, message_queue_size: int = 1000):
...
class MessageAnnouncer:
    ...
    @classmethod
    def get_or_create_streamer(cls, uuid: str) -> SSEStreamer:
        # We lock it here to avoid getting a streamer which is being deleted by `cleanup_streamers`
        # in another thread.
        with cls.streamer_lock:
            if uuid not in cls.streamers:
                cls.streamers[uuid] = SSEStreamer()
    ...
    def remove_streamer(cls, uuid: str):
        """Remove streamer, this is thread safe because we only call it inside `cleanup_streamers`"""
