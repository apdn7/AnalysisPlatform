...
def calculate_kde_trace_data(plotdata, bins=128, height=1, full_array_y=None):
    ...
    data = plotdata[ARRAY_Y] if full_array_y is None else full_array_y

    data = pd.Series(data).dropna().convert_dtypes()
    ...
    # convert data to float before proceeding because numpy couldn't understand `Float64` or `Int64`
