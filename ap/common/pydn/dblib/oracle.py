import contextlib
import datetime as dt
import logging
import re

import oracledb
from dateutil import tz

from ap.common.common_utils import strip_all_quote

logger = logging.getLogger(__name__)


class Oracle:
    """Oracle database connection and operations handler.

    Provides methods for connecting to and interacting with Oracle databases,
    including table management, query execution, and schema operations.

    Attributes:
        host: Database server hostname or IP address.
        port: Database server port number (default 1521).
        dbname: Service name for the Oracle database.
        schema: Optional schema name (defaults to username if not specified).
        username: Database user for authentication.
        password: Password for database authentication.
        is_connected: Connection status flag.
        connection: Active database connection object.
        read_only: If True, restricts to read-only operations.

    Methods:
        connect: Establishes connection to the database.
        disconnect: Closes the database connection.
        run_sql: Executes SQL query and returns results.
        fetch_many: Executes query and yields results in batches.
        list_tables: Returns list of table names.
        list_tables_and_views: Returns list of tables and views.
        list_table_columns: Returns column information for a table.
        get_timezone: Retrieves database timezone information.
        execute_sql: Executes SQL statement with commit.
        convert_sql: Converts datetime literals to Oracle format.

    Generated by Duo.
    """

    # dbname = service_name, but we want to have the same arguments name using in `ReadOnlyDbProxy`
    def __init__(self, host, dbname, username, password, schema=None, port=1521, read_only=False) -> None:
        self.host = host
        self.port = port
        self.dbname = dbname
        self.schema = schema
        self.username = username

        # username equal schema, don't use the schema, we use `username` as schema already.
        if username == schema:
            self.schema = None

        self.password = password
        self.is_connected = False
        self.read_only = read_only

    def dump(self):
        logger.info(
            f"""\
===== DUMP RESULT =====
DB Type: Oracle
self.host: {self.host}
self.port: {self.port}
self.dbname: {self.dbname}
self.username: {self.username}
self.schema: {self.schema}
self.is_connected: {self.is_connected}
=======================
""",
        )

    def connect(self):
        dsn = oracledb.makedsn(self.host, self.port, service_name=self.dbname)
        try:
            self.connection = oracledb.connect(
                user=self.username,
                password=self.password,
                dsn=dsn,
                # no need to pass encoding
                # https://python-oracledb.readthedocs.io/en/latest/user_guide/globalization.html#setting-the-client-character-set
            )

            # specify which schema to set
            # use `schema` if user sets them.
            # Otherwise, we must use the current schema,
            # there is a bug that user logs in `USER` but the `CURRENT_SCHEMA` is different user !!
            # see: https://gitlab.com/dot-asterisk/biz-app/analysis-interface/analysisinterface/-/issues/49
            schema = self.schema if self.schema else self.username
            sql = f'ALTER SESSION SET CURRENT_SCHEMA = {schema}'
            cur = self.connection.cursor()
            cur.execute(sql)
            self.connection.commit()

            self.is_connected = True
            return self.connection
        except Exception as e:
            logger.exception(e)
            return False

    def disconnect(self):
        if not self._check_connection():
            return False
        self.connection.close()
        self.is_connected = False

    # As of now, create_table, drop_table, and intert_table aren't added
    # because we want to use tables of Oracle as "Read-only"
    def create_table(self, tblname, valtypes):
        if not self._check_connection():
            return False
        sql = f'create table {tblname:s}('
        for idx, val in enumerate(valtypes):
            if idx > 0:
                sql += ','
            sql += val['name'] + ' ' + val['type']
        sql += ')'
        logger.info(sql)
        cur = self.connection.cursor()
        cur.execute(sql)
        cur.close()
        self.connection.commit()
        logger.info(f'{tblname} created!')

    # テーブル名の配列を取得
    def list_tables(self):
        if not self._check_connection():
            return []
        sql = 'select table_name from user_tables'
        cur = self.connection.cursor()
        cur.execute(sql)
        results = []
        rows = cur.fetchall()
        for row in rows:
            results.append(row[0])
        cur.close()
        return results

    def list_tables_and_views(self):
        if not self._check_connection():
            return False

        # if user specified the schema AND the schema is not equals the username
        # the we should query from `ALL_VIEWS` and `ALL_TABLES`.
        # Otherwise, we should query from `USER_VIEWS` and `USER_TABLES`
        if self.schema:
            sql = f"""
        SELECT VIEW_NAME AS NAME
        FROM ALL_VIEWS
        WHERE OWNER = '{self.schema}'
        UNION ALL
        SELECT TABLE_NAME AS NAME
        FROM ALL_TABLES
        WHERE OWNER = '{self.schema}'
        ORDER BY NAME
        """
        else:
            sql = """
SELECT VIEW_NAME AS NAME
FROM USER_VIEWS
UNION ALL
SELECT TABLE_NAME AS NAME
FROM USER_TABLES
ORDER BY NAME
"""

        cur = self.connection.cursor()
        cur.execute(sql)
        results = []
        rows = cur.fetchall()
        for row in rows:
            results.append(row[0])
        cur.close()
        return results

    def drop_table(self, tblname):
        if not self._check_connection():
            return False
        sql = 'drop table ' + tblname
        cur = self.connection.cursor()
        try:
            cur.execute(sql)
            self.connection.commit()
            logger.info(f'{tblname} dropped!')
        except Exception as e:
            logger.exception(e)
        finally:
            cur.close()

    # テーブルのカラムのタイプを辞書の配列として返す(元々はtbl_get_valtypes関数)
    # columns:
    #  name => カラム名,
    #  type => カラムタイプ

    def list_table_columns(self, tblname: str):
        if not self._check_connection():
            return False
        # this sql can work with views as well
        sql = f"""
select column_id, column_name, data_type
from ALL_TAB_COLUMNS
where table_name='{tblname}'
"""
        cur = self.connection.cursor()
        cur.execute(sql)
        rows = cur.fetchall()
        results = []
        for row in rows:
            results.append({'name': row[1], 'type': row[2]})
        cur.close()
        return results

    # list_table_columnsのうちcolumn nameだけ必要な場合(元はtbl_get_colnames)
    # def list_table_colnames(self, tblname):
    #     if not self._check_connection():
    #         return False
    #     columns = self.list_table_columns(tblname)
    #     colnames = []
    #     for column in columns:
    #         colnames.append(column['name'])
    #     return colnames

    def divide_data_to_chunks(self, lst_data, n):
        """Devide a list to chunks of n elements.

        :param lst_data: data needs to be chunked
        :param n: size of chunk
        :return: an array of chunks with max size of n
        """
        for i in range(0, len(lst_data), n):
            yield lst_data[i : i + n]

    def insert_table_records(self, tblname, names, values, add_comma_to_value=True):
        """Insert one or many records to Oracle DB.

        Note: Please note that
            - TIMESTAMP literal should be in '%Y-%m-%d %H:%M:%S' format.
            - DATE literal should be in '%Y-%m-%d' format.

        :param tblname: Table name
        :param names: Column names
        :param values: records need to be inserted
        :return: None
        """
        if not self._check_connection():
            return False

        # Generate column names field
        col_name = '('
        for idx, name in enumerate(names):
            if idx > 0:
                col_name += ','
            col_name += name
        col_name += ') '

        # Generate values field
        data_chunks = self.divide_data_to_chunks(values, 10_000)

        for chunk in data_chunks:
            sql = 'insert all '
            for idx1, value in enumerate(chunk):
                sql += f' into {tblname:s} ' + col_name + ' values ('
                for idx2, name in enumerate(names):
                    if idx2 > 0:
                        sql += ','

                    if value[name] in ('', None):
                        sql += 'Null'
                    elif add_comma_to_value:
                        sql += "'" + str(value[name]) + "'"
                    else:
                        sql += str(value[name])

                sql += ')'
            sql += ' SELECT 1 FROM dual'
            cur = self._create_cursor_with_date_time_format()
            sql = Oracle.convert_sql(sql)
            cur.execute(sql)
            cur.close()
            logger.info(f'Inserted {len(chunk)} rows')
        self.connection.commit()

        logger.info(f'Dummy data was inserted to {tblname}')

    # SQLをこのまま実行
    # [以下、結果をDict形式で返す方法]
    # https://stackoverflow.com/questions/16519385/
    # output-pyodbc-cursor-results-as-python-dictionary
    # cols, rows = db1.run_sql("select * from tbl01")
    # という形で呼び出す
    def run_sql(self, sql, row_is_dict=True):
        if not self._check_connection():
            return False
        logger.info(sql)
        cur = self._create_cursor_with_date_time_format()
        sql = Oracle.convert_sql(sql)

        cursor = cur.execute(sql)
        # cursor.descriptionはcolumnの配列
        # そこから配列名(column[0])を取り出して配列columnsに代入
        cols = [column[0] for column in cursor.description]
        # columnsは取得したカラム名、rowはcolumnsをKeyとして持つ辞書型の配列
        # rowは取得したカラムに対応する値が順番にrow[0], row[1], ...として入っている
        # それをdictでまとめてrowsに取得
        rows = [dict(zip(cols, row, strict=False)) for row in cur.fetchall()] if row_is_dict else cur.fetchall()

        cursor.close()
        return cols, rows

    def fetch_many(self, sql, size=10_000, params=None):
        if not self._check_connection():
            return False

        cur = self._create_cursor_with_date_time_format()
        sql = Oracle.convert_sql(sql)

        cursor = cur.execute(sql, **params) if params is not None else cur.execute(sql)

        cols = [column[0] for column in cursor.description]
        yield cols
        while True:
            rows = cur.fetchmany(size)
            if not rows:
                break

            yield rows

        cursor.close()

    def execute_sql(self, sql):
        """For executing any query requires commit action
        :param sql: SQL to be executed
        :return: Execution result
        """
        if not self._check_connection():
            return False

        cur = self._create_cursor_with_date_time_format()
        sql = Oracle.convert_sql(sql)
        logger.info(sql)
        try:
            cur.execute(sql)
            self.connection.commit()
        except Exception:
            return False
        finally:
            cur.close()
        return True

    # private functions
    def _check_connection(self):
        if self.is_connected:
            return True
        # 接続していないなら
        logger.warning('Connection is not Initialized. Please run connect() to connect to DB')
        return False

    def _create_cursor_with_date_time_format(self):
        if not self.is_connected:
            return None
        cur = self.connection.cursor()
        try:
            cur.execute("alter session set nls_date_format = 'YYYY-MM-DD HH24:MI:SS'")
            cur.execute("alter session set nls_timestamp_format = 'YYYY-MM-DD HH24:MI:SS.FF3'")
        except Exception:
            pass

        return cur

    @staticmethod
    def convert_sql(sql):
        """Convert filter datetime before execute sql

        Arguments:
            sql {[type]} -- [description]

        Returns:
            [type] -- [description]
        """
        timestamp_frm = "'YYYY-MM-DD HH24:MI:SS.FF3'"
        timestamp_z_frm = "'YYYY-MM-DD HH24:MI:SS.FF3TZR'"
        # timestamp_z_frm_6 = "'YYYY-MM-DD HH24:MI:SS.FF6TZR'"
        timestamp_tz_frm = "'YYYY-MM-DD HH24:MI:SS.FF3 TZH:TZM'"
        regex_str = r"('\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}.\d{3}')"
        regex_z_str = r"('\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}.\d{3}Z')"
        # regex_z_str_6 = r"('\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}.\d{6}Z')"
        regex_tz_str = r"('\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}.\d{3}\s[+-]?\d{1,2}:\d{2}')"
        new_sql = re.sub(regex_str, f'TO_TIMESTAMP(\\1,{timestamp_frm})', sql)
        # new_sql = re.sub(regex_z_str_6, f'TO_TIMESTAMP_TZ(\\1,{timestamp_z_frm_6})', new_sql)
        new_sql = re.sub(regex_z_str, f'TO_TIMESTAMP_TZ(\\1,{timestamp_z_frm})', new_sql)
        new_sql = re.sub(regex_tz_str, f'TO_TIMESTAMP_TZ(\\1,{timestamp_tz_frm})', new_sql)
        # new_sql = re.sub(regex_str, f'TO_CHAR(TO_TIMESTAMP(\\1,{timestamp_frm}),{timestamp_frm})', sql)
        return new_sql

    def get_data_type_by_colname(self, tbl, col_name):
        col_name = strip_all_quote(col_name)
        cols = self.list_table_columns(tbl)
        data_type = [col['type'] for col in cols if col['name'] == col_name]
        return data_type[0] if data_type else None

    def is_timezone_hold_column(self, tbl, col):
        data_type = self.get_data_type_by_colname(tbl, col)

        return 'TIME ZONE' in data_type.upper()

    def get_timezone(self):
        try:
            _, rows = self.run_sql('SELECT DBTIMEZONE FROM DUAL', row_is_dict=False)
            timezone_val = rows[0][0]
            timezone = get_tzinfo_from_timezone_or_offset(timezone_val)
            return timezone
        except Exception:
            return None

    def format_tz_column(self, tbl: str, col: str, orig_col: str) -> str:
        # oracle lose the timezone information during query
        # https://github.com/oracle/python-oracledb/issues/274
        # we must do it manually
        if self.is_timezone_hold_column(tbl, orig_col):
            return f"TO_CHAR({col}, 'YYYY-MM-DD HH24:MI:SS.FF3 TZH:TZM')"
        return col


def get_tzinfo_from_timezone_or_offset(timezone_or_offset: str | None) -> dt.tzinfo | None:
    """Oracle sometimes return +09:00 without specified timezone name,
    this would cause timezone_val failed to run.
    We need to get the _correct_ name for this as well.

    >>> get_tzinfo_from_timezone_or_offset('+09:00')
    tzstr('UTC+09:00')
    >>> get_tzinfo_from_timezone_or_offset('Asia/Tokyo')
    tzfile('Japan')
    >>> get_tzinfo_from_timezone_or_offset('invalid')
    """
    if timezone_or_offset is None:
        return None

    timezone_val = tz.gettz(timezone_or_offset)
    if timezone_val is not None:
        return timezone_val

    # try parse from "%z"
    with contextlib.suppress(ValueError):
        time_data = dt.datetime.strptime(timezone_or_offset, '%z')
        return tz.gettz(time_data.tzname())

    return None
