...
class EventListener:
    ...
    def reschedule_job(event: Event):
        if isinstance(event, EventRescheduleJob):
            scheduler.reschedule_job(
                id=event.job_id,
                func=event.function,
                func_params=event.kwargs,
                reschedule_seconds=event.reschedule_seconds,
            )
    ...
    @staticmethod
    def kill_job(event: Event):
        if isinstance(event, EventKillJobs):
            kill_jobs(job_types=event.job_types, process_id=event.process_id)
