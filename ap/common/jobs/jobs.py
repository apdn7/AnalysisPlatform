...
class JobKwargs(BaseModel):
    ...
    job_id: str | None = Field(None, alias='_job_id')
    job_name: str | None = Field(None, alias='_job_name')
    job_added_at: datetime = Field(alias='_job_added_at', default_factory=lambda: datetime.now(utc))
    proc_id: int | None = Field(None, alias='process_id')
...
class RunningJobStatus(BaseEnum):
    """Enumeration for tracking the execution status of running jobs.

    Defines the possible states:
    - EXECUTED: Job has completed execution
...
class RunningJob(BaseModel):
    """Model representing a job currently running in the application.

    Tracks the state and metadata of jobs being executed by the scheduler.

    Attributes:
        id: Unique identifier for the job.
        name: Human-readable name of the job.
        proc_id: Optional process ID associated with the job.
        added_at: Timestamp when the job was added to the scheduler.
        run_at: Timestamp when the job was scheduled to run.
        wait_to_kill: Flag indicating if the job is waiting to be terminated.
        status: Current execution status of the job.

    Methods:
        modified: Checks if the job was modified after being added.
        update: Updates job information and synchronizes with running_jobs.
    ...
    proc_id: int | None = ConfigDict(coerce_numbers_to_str=True)
    ...
    def update(
        ...
        wait_to_kill: bool | None = None,
        status: RunningJobStatus | None = None,
        running_jobs: DictProxy[str, RunningJob] | None = None,
    ):
