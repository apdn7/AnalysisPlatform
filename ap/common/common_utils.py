...
class PostgresFormatStrings(Enum):
    ...
    This enum defines standard datetime format strings used for PostgreSQL
    database operations, including date, time, and datetime formats.

    Attributes:
        DATE: Date format string (%Y-%m-%d).
        TIME: Time format string without milliseconds (%H:%M:%S).
        DATETIME: Datetime format string with microseconds (%Y-%m-%d %H:%M:%S.%f).
...
class SQLiteFormatStrings(Enum):
    ...
    This enum defines standard datetime format strings used for SQLite
    database operations, including date, time, and datetime formats.
...
def universal_db_exists():
    ...
    Path(instance_folder).mkdir(parents=True, exist_ok=True)
...
def convert_sa_sql_to_sa_str(fn):
    @wraps(fn)
    def decorator(self, *args: Any, **kwargs: Mapping[str, Any]):
        sig = inspect.signature(fn)
        param_names = list(sig.parameters.keys())
        # remove `self`
        param_names = param_names[1:]
        for arg_name, arg_value in zip(param_names, args, strict=False):
...
def handle_read_only(check_sql_statement: bool = True):
    def decorator(fn):
        @wraps(fn)
        def wrapper(self, *args: Any, **kwargs: Mapping[str, Any]):
...
def gen_sql_like_value(val, func: FilterFunc, position=None):
    ...
    if func is FilterFunc.SUBSTRING:
        if position is None:
            position = 1
        return [UNDER_SCORE * max(0, (position - 1)) + val + '%']
...
class NoDataFoundException(Exception):
    """Exception raised when no data is found in a query or operation.

    This custom exception is used throughout the application to signal
    that a data retrieval operation returned no results.

    Attributes:
        code: Error code (999) identifying this exception type.
...
def gen_export_history_table_name(proc_id: int):
    return f't_export_history_{proc_id}'


def gen_bridge_column_name(id, name):
    name = to_romaji(name)
    # clear column name
    name = re.sub(r'[^A-Za-z0-9_]', UNDER_SCORE, name)
...
def convert_to_datetime(string_time: str | None, timezone: tzinfo | None = None) -> datetime | None:
...
class BoundType(Enum):
    """Enumeration for boundary types in range operations.

    This enum defines the types of boundaries used in range operations,
    indicating whether a boundary value is included, excluded, or unbounded.

    Attributes:
        INCLUDED: Boundary value is included in the range.
        EXCLUDED: Boundary value is excluded from the range.
        UNBOUNDED: No boundary (infinite range).
...
class Bound[T]:
    ...
    def __init__(self, kind: BoundType, value: T | None = None) -> None:
        """Initialize a Bound instance with a boundary type and optional value.

        Args:
            kind: The type of boundary (INCLUDED, EXCLUDED, or UNBOUNDED).
            value: The boundary value. If None, the bound is treated as UNBOUNDED.
    ...
    @classmethod
    def included(cls, value: T) -> Bound[T]:
        return Bound(BoundType.INCLUDED, value)

    @classmethod
    def excluded(cls, value: T) -> Bound[T]:
        return Bound(BoundType.EXCLUDED, value)

    @classmethod
    def unbounded(cls) -> Bound[T]:
        return Bound(BoundType.UNBOUNDED)
    ...
    def __repr__(self) -> str:
        ...
        Returns:
            A string showing the bound type and value, or 'Unbounded' if unbounded.
    ...
    def __eq__(self, other: Bound[T]) -> bool:
        """Check equality between two Bound instances.

        Args:
            other: Another Bound instance to compare with.

        Returns:
            True if both bounds have the same kind and value, False otherwise.
    ...
    def __hash__(self) -> int:
        """Compute the hash value of the Bound instance.

        Returns:
            Hash value based on the bound's kind and value.
...
class RangeBound[T]:
    """Supper class for range bound."""

    min: Bound[T]
    max: Bound[T]

    def map(self, func: Callable[[T], U]) -> RangeBound[U]:
        return RangeBound(self.min.map(func), self.max.map(func))
    ...
    def __eq__(self, other: RangeBound[T]) -> bool:
        """Check equality between two RangeBound instances.

        Args:
            other: Another RangeBound instance to compare with.

        Returns:
            True if both ranges are equal (same type, both empty, or same min/max), False otherwise.
    ...
    def __hash__(self) -> int:
        """Compute the hash value of the RangeBound instance.

        Returns:
            Hash value based on the range's min and max bounds.
    ...
    def __str__(self) -> str:
        """Return a string representation of the RangeBound instance.

        Returns:
            String representation using the repr() method.

        Generated by Duo
        """
        return repr(self)

    def complements(self) -> list[RangeBound[T]]:
        ...
        if self.min.kind is BoundType.UNBOUNDED:
            # [x, inf)
            return [self.__class__(min=self.max.reverse(), max=Bound.unbounded())]

        # (x, inf)
        if self.max.kind is BoundType.UNBOUNDED:
            # (-inf, x]
            return [self.__class__(min=Bound.unbounded(), max=self.min.reverse())]

        # (a, b)
        return [
            # (-inf, a]
            self.__class__(min=Bound.unbounded(), max=self.min.reverse()),
            # [b, inf)
            self.__class__(min=self.max.reverse(), max=Bound.unbounded()),
        ]

    def different(self, other: Self) -> list[Self]:
    ...
    def intersect(self, other: RangeBound[T]) -> Self | None:
        ...
        if self.max == other.max:
            max_bound = self.max
        elif self.max.kind is BoundType.UNBOUNDED:
            max_bound = other.max
        elif other.max.kind is BoundType.UNBOUNDED:
            max_bound = self.max
        elif self.max.value == other.max.value:
            # self.max != other.max ==> one of them must be `excluded`
            max_bound = Bound.excluded(self.max.value)
        elif self.max.value < other.max.value:
            max_bound = self.max
        elif self.max.value > other.max.value:
            max_bound = other.max
        ...
        if self.min == other.min:
            min_bound = self.min
        elif self.min.kind is BoundType.UNBOUNDED:
            min_bound = other.min
        elif other.min.kind is BoundType.UNBOUNDED:
            min_bound = self.min
        elif self.min.value == other.min.value:
            # self.min != other.min ==> one of them must be `excluded`
            min_bound = Bound.excluded(self.min.value)
        elif self.min.value > other.min.value:
            min_bound = self.min
        elif self.min.value < other.min.value:
            min_bound = other.min
        ...
        time_range = self.__class__(min=min_bound, max=max_bound)

        return None if time_range.is_empty() else time_range

    def get_start_operator(self) -> str:
        """Get SQL operator for start bound"""
        if self.min.kind == BoundType.INCLUDED:
            return '>='
        elif self.min.kind == BoundType.EXCLUDED:
            return '>'
        else:
            return ''

    def get_end_operator(self) -> str:
        """Get SQL operator for end bound"""
        if self.max.kind == BoundType.INCLUDED:
            return '<='
        elif self.max.kind == BoundType.EXCLUDED:
            return '<'
        else:
            return ''


class TimeRangeStr(RangeBound[str]):
    ...
    def format(self) -> str:
        ...
        return f'{self.min.value}{UNDER_SCORE}{self.max.value}'


class TimeRange(RangeBound[dt.datetime]):
    """Time range for common querying: Bound(min), Bound(max)
    Input passed into TimeRange will be parsed to `dt.datetime`
    """

    def __init__(self, *, min: Bound[Any], max: Bound[Any]) -> None:
        """Initialize a TimeRange instance with minimum and maximum bounds.

        Args:
            min: The minimum bound of the time range.
            max: The maximum bound of the time range.

        Generated by Duo
        """
        is_min_unbounded = min.kind is BoundType.UNBOUNDED or not min.value
        is_max_unbounded = max.kind is BoundType.UNBOUNDED or not max.value

        # We don't support (-inf, inf), this should be converted as empty
        if is_min_unbounded and is_max_unbounded:
            # we use a trick here to ensure emptiness e.g: (0, 0) is an empty range
            self.min = Bound.excluded(dt.datetime.min)
            self.max = Bound.excluded(dt.datetime.min)
            return

        if is_min_unbounded:
            self.min = Bound.unbounded()
        else:
            self.min = min.map(TimeRange._parse_datetime)

        if is_max_unbounded:
            self.max = Bound.unbounded()
        else:
            self.max = max.map(TimeRange._parse_datetime)

    def with_timezone(self, timezone: str) -> Self:
        return self.__class__(
            min=self.min.map(lambda m: m.astimezone(timezone)),
            max=self.max.map(lambda m: m.astimezone(timezone)),
        )

    @staticmethod
    def _parse_datetime(
        data: Union[dt.datetime, pd.Timestamp, str, Any] | None,
    ) -> dt.datetime | None:
        if data is None:
            return data
...
class WebAuthenticationType(Enum):
    """Enumeration for web API authentication types.

    This enum defines the supported authentication methods for web API
    connections, including no authentication and basic authentication.
