...
class CustomRotatingNamer:
    """Custom namer for rotating log files with process ID and datetime formatting.

    Generates standardized log file names incorporating process IDs and timestamps
    for both timed and size-based log rotation.

    Attributes:
        pid: Process ID to include in the log file name.
        datetime_format: Format string for datetime in log file names.

    Methods:
        __call__: Transforms the default log file name to a custom format.

    Generated by Duo.
    """

    def __init__(self, pid: str, datetime_format: str) -> None:
        self.pid = pid
...
def rotator_wrapper(
    zip_create_caller: Callable[[Path], None] | None = None,
    zip_delete_caller: Callable[[Path], None] | None = None,
    ...
    def inner(source: str, dest: str) -> None:
        ...
        if zip_create_caller is not None:
            result = zip_create_caller(path.parent)
            # If the caller returns a generator, consume it
    ...
    def recompute_rollover_at(self):
        ...
        if (self.when == 'MIDNIGHT' or self.when.startswith('W')) and not self.utc:
            ...
            if dst_now != dst_at_rollover:
                if not dst_now:  # noqa DST kicks in before next rollover, so we need to deduct an hour
                    addend = -3600
...
class CustomMemoryHandler(MemoryHandler):
    """Custom memory-based log handler with configurable flush triggers.

    Extends the standard MemoryHandler to support flushing based on buffer capacity,
    record size, and time intervals.

    Attributes:
        record_max_bytes: Maximum size in bytes for a single log record before flushing.
        flush_interval: Time interval in seconds between automatic flushes.
        flush_at: Timestamp for the next scheduled flush.

    Methods:
        shouldFlush: Determines if the buffer should be flushed based on multiple criteria.
