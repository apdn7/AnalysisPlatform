...
from ap.common.constants import USER_SETTING_VERSION, DataTypeEncode, EN_DASH
...
def convert_user_setting():
    ...
    try:
        ...
        from ap.setting_module.models import make_session, CfgUserSetting, CfgProcessColumn
        ...
        with make_session() as meta_session:
            ...
            for user_setting in user_settings:
                ...
                if "version" not in settings or settings["version"] < USER_SETTING_VERSION:
                    ...
                    setting_forms = {"version": USER_SETTING_VERSION}
                    for form_key in form_keys:
                        ...
                        for setting_item in settings[form_key]:
                            ...
                            if "name" in setting_item:
                                # check datatype
                                if (
                                    setting_item["name"].startswith("GET02_VALS_SELECT")
                                    and setting_item["value"] not in except_key_name
                                ):
                                    variable_id = setting_item["value"]
                                    data_type_name = "dataType-{}".format(variable_id)
                                    has_data_type = get_setting_item_by_name(
                                        settings[form_key], data_type_name
                                    )

                                    setting_forms[form_key].append(setting_item)
                                    if not has_data_type:
                                        # find sensor
                                        variable = meta_session.query(CfgProcessColumn).get(
                                            variable_id
                                        )
                                ...
                                if setting_item["name"] == "DATETIME_RANGE_PICKER":
                                    update_setting_item["value"] = re.sub(
                                        "[~]", EN_DASH, setting_item["value"]
                                    )
                    ...
                    update_setting.settings = json_dumps(setting_forms)
...
def convert_user_setting_url():
    ...
    from ap.setting_module.models import make_session, CfgUserSetting
