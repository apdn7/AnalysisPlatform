...
class ProcLink(db.Model):
    ...
    @staticmethod
    def find_proc_link_class(link_key):
        if isinstance(link_key, str):
            keys = link_key.split(TRACING_KEY_DELIMITER_SYMBOL)
        else:
            keys = link_key

        hash_number = sum([int(key) for key in keys])
        idx = int(hash_number) % PARTITION_TABLE
        return PROC_LINK_CLASSES[idx]

    @staticmethod
    def get_first_cls():
        return PROC_LINK_CLASSES[0]


# Index('ix_link_value', ProcLink.link_value, ProcLink.link_key, ProcLink.time)

# class ProcLinkMetaData(db.Model):
#     __tablename__ = 't_proc_link_metadata'
#     process_id = db.Column(db.Integer(), primary_key=True)
#     link_key = db.Column(db.Text(), primary_key=True)
#     done_cycle_id = db.Column(db.Integer())
...
class CyclePartition:
    ...
    @classmethod
    def gen_auto_global_id(cls, proc_id, null_only=True):
        # update cycle that global id is None
        data = cls.query.filter(cls.process_id == proc_id)
        if null_only:
            data = data.filter(cls.global_id.is_(None))

        data.update({cls.global_id: cls.id})
...
class SensorTypePartition:
    ...
    @classmethod
    def get_max_value(cls, sensor_name):
        data = db.session.query(cls.value).join(Sensor).filter(Sensor.column_name == sensor_name)
        data = data.with_entities(func.max(cls.value).label('max_time'))
