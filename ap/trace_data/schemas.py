...
class EndProc:
    """Manages end process configuration with columns.

    Represents an end process in the trace data system, managing column IDs,
    names, and display names for both regular and sensor-only columns.

    Attributes:
        proc_id: Process ID.
        col_ids: List of column IDs.
        col_names: List of column names.
        col_show_names: List of column display names.
        col_sensor_only_ids: List of sensor-only column IDs.
        cfg_proc: Configuration process object.

    Methods:
        add_cols: Add columns to the process configuration.
        add_sensor_col_ids: Add sensor column IDs to the sensor-only list.
        get_col_ids: Get list of column IDs.
        get_sensor_col_ids: Get list of sensor-only column IDs.
    ...
    def __init__(self, cfg_proc: CfgProcess, cols) -> None:
...
class CategoryProc:
    """Handles category process configuration.

    Manages category process information including process ID and associated
    column IDs, names, and display names.
    ...
    def __init__(self, dic_proc_cfgs, proc, cols) -> None:
        if proc:
...
class ConditionProcDetail:
    """Manages condition process details with filters.

    Represents detailed filter configuration for a condition process,
    including filter details, selection options, and column information.

    Attributes:
        cfg_filter_details: List of configuration filter detail objects.
        is_no_filter: Flag indicating no filter is applied.
        is_select_all: Flag indicating all items are selected.
        column_id: ID of the associated column.
        column_name: Name of the associated column.
    ...
    def __init__(self, dic_filter_details) -> None:
...
class ConditionProc:
    """Process condition management with filter dictionaries.

    Manages condition process configuration with filter mappings by both
    column name and column ID.

    Attributes:
        proc_id: Process ID.
        dic_col_name_filters: Dictionary mapping column names to filter details.
        dic_col_id_filters: Dictionary mapping column IDs to filter details.
    ...
    def __init__(self, proc, condition_details: list[ConditionProcDetail]) -> None:
        self.proc_id = int(proc)
...
class CommonParam:
    """Common parameters for data processing.

    Large class containing extensive parameters for data processing operations,
    including time ranges, heatmap settings, serial processing, conditions,
    outlier removal, and various analysis options.

    Attributes:
        start_proc: Starting process ID.
        start_date: Start date for data range.
        start_time: Start time for data range.
        end_date: End date for data range.
        end_time: End time for data range.
        client_timezone: Client timezone string.
        hm_step: Heatmap step parameter.
        hm_mode: Heatmap mode parameter.
        hm_function_real: Heatmap function for real values.
        hm_function_cate: Heatmap function for categorical values.
        hm_trim: Heatmap trim parameter.
        x_option: X-axis option.
        serial_processes: List of serial process IDs.
        serial_columns: List of serial column IDs.
        serial_orders: List of serial order values.
        cond_procs: List of condition processes.
        traincond_procs: List of training condition processes.
        cate_procs: List of category processes.
        threshold_boxes: List of threshold box IDs.
        cat_exp: List of category expression IDs.
        is_validate_data: Flag for data validation.
        is_remove_outlier: Flag for outlier removal.
        remove_outlier_objective_var: Flag for removing outliers from objective variable.
        remove_outlier_explanatory_var: Flag for removing outliers from explanatory variable.
        abnormal_count: Flag for abnormal count.
        sensor_cols: List of sensor column IDs.
        is_export_mode: Flag for export mode.
        is_latest: Flag for latest data.
        judge_var: Judge variable ID.
        ng_condition: NG condition string.
        ng_condition_val: NG condition value.
        is_nominal_scale: Flag for nominal scale.
        nominal_vars: List of nominal variable IDs.
        cate_col_ids: List of category column IDs.
        outliers: Outliers data.
        is_order_by_time: Flag for ordering by time.
...
class DicParam:
    """Dictionary parameter management for processes.

    Manages comprehensive parameter dictionaries for process configuration,
    including chart settings, common parameters, process configurations,
    and trace graph information.

    Attributes:
        chart_count: Number of charts.
        common: Common parameters object.
        array_formval: List of end process configurations.
        dic_proc_cfgs: Dictionary of process configurations by ID.
        trace_graph: Trace graph object.
        dic_card_orders: Dictionary of card orders.
        cyclic_terms: List of cyclic terms.
        dic_col_cfgs: Dictionary of column configurations by ID.

    Methods:
        search_end_proc: Search for end process by ID.
        is_end_proc: Check if process ID is an end process.
        add_proc_to_array_formval: Add process to array formval.
        add_start_proc_to_array_formval: Add start process to array formval.
        add_cate_procs_to_array_formval: Add category processes to array formval.
        add_cond_procs_to_array_formval: Add condition processes to array formval.
        add_cat_exp_to_array_formval: Add category expressions to array formval.
        add_column_to_array_formval: Add columns to array formval.
        get_end_cols: Get end columns for a process.
        get_all_end_col_ids: Get all end column IDs.
        get_start_proc: Get start process ID.
        get_client_timezone: Get client timezone.
        get_facet_var_cols_name: Get facet variable column names.
        get_div_cols_label: Get division column label.
        add_datetime_col_to_start_proc: Add datetime column to start process.
        get_all_target_cols: Get all target columns.
        add_agp_color_vars: Add AGP color variables.
        get_col_info_by_id: Get column information by ID.
        get_color_id: Get color ID for target variable.
        get_color_info: Get color information for target variable.
        get_col_cfgs: Get column configurations by IDs.
        get_col_cfg: Get column configuration by ID.
        gen_label_from_col_id: Generate label from column ID.
        add_ng_condition_to_array_formval: Add NG condition to array formval.
        add_function_cols_to_sensor_cols: Add function columns to sensor columns.
        get_process_by_id: Get process by ID.
        get_all_end_procs: Get all end processes.
