...
;(function (global, factory) {
    ...
    var jexcel = (function(el, options) {
        ...
        var defaults = {
            ...
            // Allow row delete
            allowDeleteRow:true,
            // Allow row delete by keyboard
            allowManualDeleteRow:true,
            // Allow deleting of all rows
            allowDeletingAllRows:false,
            // Allow column delete
            allowDeleteColumn:true,
            // Allow column delete by keyboard
            allowManualDeleteColumn:true,
            // Allow rename column
        ...
        */
        ...
        obj.init = function() {
            ...
            el.addEventListener('focus', function(e) {
                if (jexcel.current && ! jexcel.current.selectedCell) {
                    jexcel.current.updateSelectionFromCoords(0,0,0,0);
                    jexcel.current.left();
                }
            });
        ...
        // Custom to freeze order column
        obj.updateFreezePosition = function() {
            scrollLeft = obj.content.scrollLeft;
            var width = 0;
		    const orderColWidth = obj.options.colWidths[0];
            if (scrollLeft > 0) {
                // Freeze order column cells
				const orderHeaderCell = obj.table.tHead.firstElementChild.firstElementChild;
				orderHeaderCell.classList.add('jexcel_freezed');
				orderHeaderCell.style.left = width + 'px';
				const orderBodyCells = obj.table.tBodies[0].querySelectorAll('td.jexcel_row');
				orderBodyCells.forEach((orderBodyCell) => {
					orderBodyCell.classList.add('jexcel_freezed');
					orderBodyCell.style.left = (scrollLeft + (i > 0 ? obj.records[j][i-1].style.width : 0)) + 'px';
				});
                ...
                for (var i = 0; i < obj.options.freezeColumns; i++) {
                    ...
                    obj.headers[i].classList.add('jexcel_freezed');
                    obj.headers[i].style.left = width + orderColWidth + 'px';
                    for (var j = 0; j < obj.rows.length; j++) {
                        if (obj.rows[j] && obj.records[j][i]) {
                            var shifted = (scrollLeft + (i > 0 ? obj.records[j][i-1].style.width : 0)) + 'px';
                            obj.records[j][i].classList.add('jexcel_freezed');
                            obj.records[j][i].style.left = shifted;
                        }
                    }
                ...
                // Release freeze status for order column cells
				const orderHeaderCell = obj.table.tHead.firstElementChild.firstElementChild;
				orderHeaderCell.classList.remove('jexcel_freezed');
				orderHeaderCell.style.left = '';
				const orderBodyCells = obj.table.tBodies[0].querySelectorAll('td.jexcel_row');
				orderBodyCells.forEach((cell) => {
					cell.classList.remove('jexcel_freezed');
					cell.style.left = '';
				});
                ...
                if (e.which == 37) {
                    ...
                    if (jexcel.current.options.editable == true) {
                        if (jexcel.current.selectedRow) {
                            if (jexcel.current.options.allowDeleteRow == true) {
                                if (jexcel.current.options.allowManualDeleteRow == true) {
                            ...
                            if (jexcel.current.options.allowDeleteColumn == true) {
                                if (jexcel.current.options.allowManualDeleteColumn == true) {
                    ...
                    if ((e.ctrlKey || e.metaKey) && ! e.shiftKey) {
                        if (e.which == 65) {
                            ...
                            // Ctrl + X
                            if (jexcel.current.options.editable == true && !((jexcel.current.selectedRow && jexcel.current.options.allowManualDeleteRow == false) || (jexcel.current.selectedHeader && jexcel.current.options.allowManualDeleteColumn == false))) {
                                jexcel.cutControls();
                            } else {
                                jexcel.copyControls();
                            }
