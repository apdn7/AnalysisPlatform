...
const formElements = {
    ...
    NO_FILTER: 'NO_FILTER',
    waveformSettingGrp: '#waveformSettingGrp',
...
const showWaveformChart = (res, setting = undefined, clearOnFlyFilter = false) => {
    ...
    if (res.array_plotdata && res.array_plotdata.length) {
        res.array_plotdata.forEach((plotData, index) => {
            ...
            const cardHtml = `
                <div class="card-border chart-wrapper waveform-plot-card" style="width: 100%; height: calc(30vw - 1rem);">
                    <div id="${chartId}" class="position-relative w-100 h-100"></div>
                </div>
            `;
            chartContainer.insertAdjacentHTML('beforeend', cardHtml);
            const waveform = new WaveformPlot(chartId, {
...
const transformFormdata = (clearOnFlyFilter = null, autoUpdate = false) => {
    ...
    return getwaveformFormData(formElements.formID, clearOnFlyFilter);
};
...
const getwaveformFormData = (formEleID, clearOnFlyFilter = null) => {
    let formData;
    if (clearOnFlyFilter) {
