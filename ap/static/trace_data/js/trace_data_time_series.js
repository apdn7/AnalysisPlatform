    ...
    const externalTooltipHandler = (context) => {
        ...
        const getThresholdInfor = (dataPoint) => {
            ...
            const filterCond = plotData.catExpBox
                ? (Array.isArray(plotData.catExpBox) ? plotData.catExpBox : [plotData.catExpBox])
                : null;
        ...
        const genDataTable = (dataPoint, outlierDict, currentThreshold) => {
            ...
            // filter
            thresholdTr += genTRItems(i18n.attribute,
                currentThreshold.filterCol, currentThreshold.filterDetail);
            thresholdTr += genTRItems('', ''); // br
            // threshold table
...
const updateThresholdsOnClick = (canvasId, clickedIdx) => {
    ...
    const filterCond = plotData.catExpBox
        ? (Array.isArray(plotData.catExpBox)
            ? plotData.catExpBox : [plotData.catExpBox])
        : null;
    const [chartInfos, chartInfosOrg] = getChartInfo(plotData, 'TIME', filterCond);
