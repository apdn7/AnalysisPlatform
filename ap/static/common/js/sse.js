...
broadcastChannelElector.awaitLeadership().then(() => {
    ...
    serverSentEventCon.onerror = (err) => {
        consoleLogDebug(`EventSource failed: ${uuid}`);
    };

    // onopen version
    serverSentEventCon.onopen = (e) => {
        consoleLogDebug(`EventSource established: ${uuid}`);
    };
 ...
 * @param {ShouldBroadcast} event.broadcastType - control how we broadcast
 */
const handleSSEMessage = ({ type, data, broadcastType = ShouldBroadcast.DEFAULT }) => {
    ...
    const shouldBroadcast =
        (broadcastType === ShouldBroadcast.DEFAULT && broadcastChannelElector.isLeader) ||
    ...
    // update transaction
    else if (type === serverSentEventType.transactionUpdated) {
        if (typeof handleSourceListener !== 'undefined') {
            handleSourceListener();
        }
    }
