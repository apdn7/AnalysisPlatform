...
def gen_heatmap_data(dic_param):
    ...
    dic_df_proc, hm_mode, hm_step, dic_col_func, df_cells, var_agg_cols, target_var_data, _ = \
        gen_heatmap_data_as_dict(graph_param, dic_param, dic_proc_cfgs, \
                                 dic_cat_filters, cat_exp, cat_procs, \
                                 use_expired_cache)

    # gen plotly data + gen array_plotdata from here
    dic_param = gen_plotly_data(dic_param, dic_df_proc, hm_mode, hm_step, dic_col_func, df_cells, var_agg_cols)

    # gen_cat_data_for_ondemand(dic_param, dic_proc_cfgs, target_var_data)
    return dic_param
    ...
    for proc_id, proc_data in dic_df_proc.items():
        for end_col in proc_data:
            ...
            if var_agg_col:
                ...
                dic_df_cate = {NA_STR if cate_value in ['nan', '<NA>'] else cate_value: df_cate
...
@abort_process_handler()
@notify_progress(60)
def gen_plotly_data(dic_param, dic_df_proc, hm_mode, hm_step, dic_col_func, df_cells, var_agg_col=None):
...
def gen_empty_df(end_col_id, var_agg_cols, ):
...
def gen_heatmap_data_as_dict(graph_param, dic_param, dic_proc_cfgs, dic_cat_filters={}, cat_exp=[], cat_procs=[],
    ...
    for idx, (proc_id, proc_config) in enumerate(dic_proc_cfgs.items()):
        ...
        if graph_param.is_end_proc(proc_id):
            dic_df_proc[proc_id] = graph_heatmap_data_one_proc(df, proc_config, graph_param, dic_col_func, var_agg_cols,
                                                               agg_cols)
