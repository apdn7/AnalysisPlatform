...
api_heatmap_blueprint = Blueprint('api_heatmap', __name__, url_prefix='/ap/api/chm')
...
def generate_heatmap():
    ...
    dic_param[COMMON][START_PROC] = (
        start_proc if start_proc else dic_param[ARRAY_FORMVAL][0][END_PROC]
    )
...
@api_heatmap_blueprint.route('/data_export/<export_type>', methods=['GET'])
def data_export(export_type):
    ...
    csv_data, csv_list_name = gen_csv_data(dic_param, delimiter=delimiter, by_cells=True)
