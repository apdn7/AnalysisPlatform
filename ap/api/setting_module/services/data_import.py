...
NA_VALUES = PANDAS_DEFAULT_NA | {'na', '-', '--', '---', '#NULL!', '#REF!', '#VALUE!', '#NUM!', '#NAME?', '0/0', '-0/0'}
...
class DbConnectionParam(pydantic.BaseModel):
    """Pydantic model for database connection parameters.

    This model validates and manages database connection parameters for various
    database types including PostgreSQL, MySQL, Oracle, SQL Server, Snowflake, and SQLite.

    Attributes:
        id: Optional database configuration ID.
        db_type: Type of database (e.g., POSTGRESQL, MYSQL, ORACLE).
        dbname: Database name.
        host: Database host address (optional for SQLite).
        port: Database port number (optional for SQLite).
        dbschema: Database schema name (optional).
        username: Database username (optional for SQLite).
        password: Database password (optional for SQLite/Snowflake).
        snowflake_authentication_type: Snowflake authentication type.
        snowflake_access_token: Snowflake access token.
        snowflake_private_key_file: Path to Snowflake private key file.
        snowflake_private_key_file_pwd: Password for Snowflake private key file.

    Methods:
        parse_int_or_none: Validator to parse port as integer or None.
        db_type_uppercase: Validator to convert db_type to uppercase.
        decrypt_password: Validator to decrypt passwords from database.
        data_source_no_id: Create CfgDataSource object without ID.
    ...
    db_type: str
    dbname: str
    host: str | None = None  # can be none if sqlite3
    port: int | None = None  # can be none if sqlite3
    dbschema: str | None = pydantic.Field(alias='schema', default=None)  # can be none if oracle?
    username: str | None = None  # can be none if sqlite3
    password: str | None = None  # can be none if sqlite3 / snowflake

    # snowflake
    snowflake_authentication_type: str | None = None
    snowflake_access_token: str | None = None
...
class WebConnectionParam(pydantic.BaseModel):
    """Pydantic model for web API connection parameters.

    This model validates and manages connection parameters for web API data sources,
    including authentication credentials.

    Attributes:
        id: Optional web connection configuration ID.
        url: Web API URL.
    ...
    Methods:
        decrypt_password: Validator to decrypt password from database if needed.
...
def write_duplicate_import(df: DataFrame, file_name_elements: list[str]):
    ...
    file_name = UNDER_SCORE.join([str(element) for element in file_name_elements if element])
...
def write_error_import(
    ...
    if csv_directory:
        ...
        file_name = proc_name + UNDER_SCORE + time_str + TXT_FILE_TYPE
        folders = []
...
@log_execution_time()
def validate_data(
    df: DataFrame, dic_use_cols: dict[str, CfgProcessColumn], exclude_cols: list[str] | None = None
) -> pd.DataFrame:
    """
    ...
    for col_name in df.columns:
        ...
        to_replaces = [NA_VALUES, INF_NEG_VALUES, INF_VALUES]
        ...
        if user_data_type == DataType.BOOLEAN.name:
            to_replaces = [NA_VALUES]
            replace_values = [nan]

        # convert `to_replaces` is lowercase because we lowercase the data before comparing
        for i in range(len(to_replaces)):
            to_replaces[i] = [str(v).lower() for v in to_replaces[i]]
        ...
        if user_data_type in [DataType.INTEGER.name, DataType.REAL.name, DataType.BOOLEAN.name]:
            ...
            if len(non_numeric_data):
                for to_replace, replace_value in zip(to_replaces, replace_values, strict=False):
                    replaceable = non_numeric_data.str.lower().isin(to_replace)
        ...
        #     df = convert_judge_values(df, dic_use_cols[col_name])
            ...
            if df[col_name].notna().any():
                for to_replace, replace_value in zip(to_replaces, replace_values, strict=False):
                    df.loc[df[col_name].str.lower().isin(to_replace), col_name] = replace_value
...
def data_pre_processing(
    ...
    # parse data type
    df = validate_data(df, dic_use_cols, exclude_cols=exclude_cols)
