...
class Autolink:
    """Service class for automatic process linking based on serial numbers and timestamps.

    This class provides functionality to automatically determine the order of processes
    by analyzing serial numbers and timestamps across different data sources (CSV, V2, databases).

    Methods:
        get_autolink_groups: Analyze processes and group them based on shared serial numbers.
        filter_dict_key_type: Filter dictionary to include only specified data types.
...
class CompletedAutolinkDataProcess:
    """Immutable data class representing completed autolink data processing.

    This class wraps the final DataFrame result after autolink data processing
    is complete.

    Attributes:
        data: DataFrame containing processed autolink data with serial, date, and ID columns.
...
class AutolinkDataProcess:
    """Manages autolink data processing for a single process.

    This class handles reading, preprocessing, and accumulating data from various
    sources (CSV, V2, databases) for autolink analysis. It extracts serial numbers
    and timestamps to determine process relationships.

    Attributes:
        DATA_TYPES: Dictionary mapping column names to pandas data types.
        process_id: ID of the process being analyzed.
        df: DataFrame accumulating autolink data.
        cfg_process: Process configuration object.
        main_date_col: Main date column configuration.
        main_time_col: Main time column configuration.
        generated_datetime_col: Generated datetime column configuration.
        main_datetime_col: Main datetime column configuration.
        main_serial_col: Main serial column configuration.

    Methods:
        can_run_autolink: Check if autolink can be run on this process.
        is_generated_datetime: Check if datetime is generated from date and time columns.
        has_enough_data: Check if enough data has been collected.
        calculate_main_function_column: Calculate function column values.
        update: Update the DataFrame with new data.
        convert_timezone: Convert datetime column to UTC timezone.
        filter_dict_key_type: Filter dictionary to include only specified data types.
        completed: Return completed autolink data process.

    Generated by Duo.
    """

    DATA_TYPES: Final[dict[str, pd.ExtensionDtype]] = {
    ...
    def __init__(self, process_id: int) -> None:
        ...
        self.main_date_col: CfgProcessColumn | None = None
        self.main_time_col: CfgProcessColumn | None = None
        self.generated_datetime_col: CfgProcessColumn | None = None
        self.main_datetime_col: CfgProcessColumn | None = None
        self.main_serial_col: CfgProcessColumn | None = None
    ...
    def update(self, df: pd.DataFrame):
        ...
        # handle convert by formula for data link
        for col in [self.main_datetime_col, self.main_date_col, self.main_time_col]:
            if col:
                ...
                if formula:
                    df[col.column_raw_name] = formula.convert(df[col.column_raw_name])
                    # remove column after convert if it is a formula column
                    del dict_datatype[col.column_raw_name]
    ...
    with pd.read_csv(filename, **read_params) as reader:
        for df in reader:
            ...
            if autolink_data.main_serial_col.is_main_serial_function_column:
                df_auto_link = autolink_data.calculate_main_function_column(df)
...
def read_autolink_db(autolink_data: AutolinkDataProcess) -> CompletedAutolinkDataProcess:
    ...
    if main_serial_col.is_main_serial_function_column:
        df = autolink_data.calculate_main_function_column(df)
...
def read_autolink_v2_single_file(file: str, autolink_datas: list[AutolinkDataProcess]):
    ...
    for autolink_data in autolink_datas:
        ...
        if autolink_data.main_serial_col.is_main_serial_function_column:
            df_one_file = autolink_data.calculate_main_function_column(
                df_one_file,
            )
