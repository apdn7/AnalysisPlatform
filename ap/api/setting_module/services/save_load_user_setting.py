...
class UserSettingDetail:
    """Represents a user setting detail with its properties and transformations.

    This class encapsulates user setting information including id, name, value,
    type, level, and other UI-related properties. It provides methods to convert
    settings to objects and check setting types.

    Attributes:
        id: Unique identifier for the setting.
        name: Name of the setting.
        value: Value of the setting.
        type: Type of the setting (e.g., checkbox, radio, select).
        level: Level or hierarchy of the setting.
        genBtnId: Generated button ID for the setting.
        checked: Whether the setting is checked (for checkboxes/radios).
        isActiveTab: Whether this setting belongs to the active tab.
        original_obj: Original dictionary object containing all values.

    Methods:
        convert_to_obj: Convert the setting to a dictionary object.
        is_checkbox_or_radio: Check if the setting is a checkbox or radio button.

    Generated by Duo
    """

    def __init__(self, dic_vals) -> None:
...
def map_form(dic_src_vals, dic_des_vals):
    ...
    if len(dic_src_vals) == len(dic_des_vals):
        names = zip(list(dic_src_vals), list(dic_des_vals), strict=False)
    else:
        src_active_form = get_active_tab(dic_src_vals)
        names = zip([src_active_form], list(dic_des_vals), strict=False)
...
def get_active_tab(dic_setting):
    ...
    zip_forms = zip(list(dic_setting), tabs, strict=False)
    for form_name, is_active in zip_forms:
        if is_active:
            return form_name

    return next(iter(dic_setting.keys()))
