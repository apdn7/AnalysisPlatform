...
class DataExport:
    ...
    def __init__(self, export_config: CfgExport) -> None:
    ...
    def export_columns(self) -> list[CfgProcessColumn]:
        ...
        selected_column_ids = sorted(detail.process_column.id for detail in self.config.export_details)
        column_ids.extend(selected_column_ids)

        columns = self.config.process.get_cols(dict.fromkeys(column_ids))
        return sorted(columns, key=lambda c: c.id)
    ...
    def sql_query(self) -> sa.Select | None:
        with DbProxy(gen_data_source_of_universal_db(self.config.process_id), True) as db_instance:
            exported_range = ExportHistoryTable.exported_range(db_instance, self.config.process_id, self.config.id)
    ...
    def export(self) -> Path | None:
        ...
        with DbProxy(gen_data_source_of_universal_db(self.config.process_id), True) as db_instance:
            ExportHistoryTable.insert(db_instance, self.config.process_id, export_history_record)
